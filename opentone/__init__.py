"""OpenTone — DTMF tone encoding and decoding in pure Python.

The package turns text or dial strings into the dual-tone multi-frequency
(DTMF) signals used by telephone keypads, writes them to WAV, and recovers
them again with a Goertzel detector. It depends only on the standard library.

Two entry points:

- :class:`ToneGenerator` — render text or a DTMF string to a WAV file.
- :class:`ToneDecoder` — recover the string from a WAV file.
